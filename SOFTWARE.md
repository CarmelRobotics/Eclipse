...
**Heading source.** The Pigeon owns rotation entirely — the fusion above is translation-only (infinite heading σ). A multi-tag vision heading seed was tried to remove the "gyro must be field-aligned" dependency, but it reversed the driver controls (it trusts the Limelight mounting yaw, which was miscalibrated) and was reverted; heading alignment is re-established the conventional way (auto start pose, or POV-down `seedFieldCentric`). See §14 incident 8. The gyro-alignment single-point-of-failure therefore still exists and is an open item (§15).
...
**8. Aiming's silent single point of failure: gyro heading (attempted fix reverted).**
*Symptom:* none observed — found by audit. Aim depends entirely on the gyro being field-aligned, and vision (by deliberate design) only corrects translation, never heading.
*Cause:* a wrong gyro heading (no seed in teleop-only practice, a mid-match bump, a bad auto seed) would offset every shot by that error with no self-correction.
*Attempted fix, reverted:* seeding the gyro from multi-tag vision heading. It **reversed the driver controls** — the seed trusts the Limelight mounting yaw, which was ~180° miscalibrated, so adopting the vision heading flipped the field-centric frame. Reverted to gyro-owns-heading; the SPOF is back on the open-items list (§15).
*Lesson:* a "correction" from a sensor is only as trustworthy as that sensor's calibration — the heading seed silently inherited the camera-yaw error and turned a latent aiming risk into an active driving failure. Separately: this surfaced a real 180° driver-forward inversion, fixed by flipping the operator-perspective constants (the alliance-safe way — negating stick signs would have double-cancelled on one alliance).
...
6. **Camera mount pose is web-UI-only — and its yaw is currently miscalibrated (~180° off).** `LocalisationConstants` documents the mount, but nothing pushes it to the camera; the heading-seed attempt (§14 incident 8) surfaced that the Limelight's yaw is wrong, which is why vision heading can't be trusted yet. Fix the yaw in the LL UI (confirm `vision/pose heading deg` matches `pose heading deg`), then `setCameraPose_RobotSpace` at startup so it's version-controlled.
7. **Gyro-heading single point of failure (open).** Aim trusts the gyro's field alignment with no self-correction (the vision heading seed that would have closed this was reverted, incident 8). Until the camera yaw is fixed and the seed re-enabled, seed heading reliably every match (auto start pose or POV-down) and watch for a wrong `pose heading deg`.
